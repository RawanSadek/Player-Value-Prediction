...
def preProcessing(data,X,Y):
    ...
    workrate = {"Low": 1, "Medium": 2, "High": 3}
    X["work_rate_attacking"] = X["work_rate_attacking"].replace(workrate)
    X["work_rate_defense"] = X["work_rate_defense"].replace(workrate)
    ...
    # Feature Selection
    X.drop(['id', 'name', 'full_name', 'birth_date', 'height_cm', 'weight_kgs', 'nationality', 'work_rate',
